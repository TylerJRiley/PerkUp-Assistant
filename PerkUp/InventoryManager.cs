...
//refactor and split up class checking again
   ...
   public List<InventoryItem> ListReturn()
   {
    ...
    public InventoryItem InputStringToItem()
    {
        ...
        while(convertedStringItem ==null)
        {
            ...
            {
                convertedStringItem = inventoryItems.FirstOrDefault(i => i.itemName.Equals(userInput, StringComparison.OrdinalIgnoreCase));
    ...
    private List<InventoryItem> FilterByCatigory(List<InventoryItem> listRef,string filterItemCatigory)
    {
        if (!string.IsNullOrWhiteSpace(filterItemCatigory))
        {
           
            return listRef.Where(i => i.itemCategory.ToLower() == filterItemCatigory.ToLower()).ToList();
        }
        else
        {
            Console.WriteLine($"List was not able to be Filterd by {filterItemCatigory}. Either that catigory does not exist or it was entred incorectly. Please check your selection again.");
            return listRef;
        }
    }
    ...
    public void InspectItemByName(InventoryItem item)
    {
        if(ListUtil.CheckListForItem(item, inventoryItems))
        {
            Console.WriteLine($"Item Information is: \n- Item ID: {item.itemID} \n- Item Name: {item.itemName} \n- Item Description: {item.itemDescription} \n- Item Catigory: {item.itemCategory} \n- Item Quantity in Scock: {item.itemStockQuantity} \n- Item Reorder Level: {item.itemReorderLevel}");
        }
    }
    ...
    public void AddToSockQuantity(string item, int quantityToAdd) //add an amoutn to the current stock amount
    {
        ...
        if( modifiyItem != null)
        {
            ...
            if(ListUtil.CheckListForItem(modifiyItem,itemsToReorder) && modifiyItem.itemStockQuantity > modifiyItem.itemReorderLevel)
            {
    ...
    public void RemoveItem(string item) //remove Items from list
    {
        ...
        var itemToRemove = inventoryItems.FirstOrDefault(i => i.itemName.ToLower() == item.ToLower());
            ...
            if (itemToRemove != null)
            {
                ...
                if (ListUtil.CheckListForItem(itemToRemove,itemsToReorder))
                {
    ...
    public int UserInputAsInt(int defultValue)
    {
